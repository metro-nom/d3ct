...
class Converter(PluginBase):
    ...
    def traverse(self, py_obj):
        PLUGIN_LOG.debug("json_connections.Converter.traverse()")
        if 'uuid' in py_obj:
            for connection in self.inp_data['connections']:
                if connection['src'] == py_obj['uuid']:
                    if 'connections' not in py_obj:
                        py_obj['connections'] = []
                    # for conn_param in connection:
                    py_obj['connections'].append(connection)
        if 'children' in py_obj:
            for child in py_obj['children']:
                self.traverse(child)

    def trans(self, py_obj):
        PLUGIN_LOG.debug("json_connections.Converter.trans()")
