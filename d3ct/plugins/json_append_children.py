...
class Converter(PluginBase):
    ...
    def traverse(self, py_obj):
        PLUGIN_LOG.debug("json_append_children.Converter.traverse()")
        if 'uuid' in py_obj:
            for obj in self.inp_data['objects']:
                if obj['uuid'] == py_obj['uuid']:
                    if 'children' not in py_obj:
                        py_obj['children'] = []
                    py_obj['children'].extend(obj['children'])
        if 'children' in py_obj:
            for child in py_obj['children']:
                self.traverse(child)

    def trans(self, py_obj):
        PLUGIN_LOG.debug("json_append_children.Converter.trans()")
